...
import { HiOutlineMail, HiOutlineLockClosed, HiOutlineUser, HiOutlineKey, HiOutlineShieldCheck } from 'react-icons/hi'
...
export default function SetupAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Tunggu toast selesai muncul, lalu redirect ke login
      setTimeout(() => {
        router.push('/login')
      }, 2000)
    } catch (error) {
      console.error('Setup admin error:', error)
      toast.error(error instanceof Error ? error.message : 'Pembuatan akun admin gagal')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <div className="space-y-4 rounded-lg bg-zinc-900/70 p-6 shadow-md border border-zinc-800">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">Kunci rahasia diperlukan untuk membuat akun admin</p>
            </div>
          </div>
