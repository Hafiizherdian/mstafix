...
export default function GenerateSoal() {
  ...
  useEffect(() => {
    const checkAuth = () => {
      try {
        ...
        if (!token || !user) {
          console.log(
            "Token atau user data tidak ditemukan, redirect ke login",
          );
        ...
        // Middleware sudah menghandle token validation
        // Jadi kita bisa langsung set loading false
        setIsLoading(false);
      } catch (error) {
        console.error("Auth check error:", error);
        console.log("Error saat cek auth, redirect ke login");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          toast.error(
            "Permintaan timeout. File terlalu besar atau format tidak didukung.",
          );
        } else if (response.status === 413) {
          toast.error("Ukuran file terlalu besar. Maksimal 8MB.");
        } else if (response.status === 415) {
          toast.error(
            "Format file tidak didukung. Gunakan TXT, PDF, DOC, atau DOCX.",
          );
        } else if (response.status === 422) {
          toast.error(
            "Konten file tidak dapat diekstrak. File mungkin berisi gambar atau terenkripsi.",
          );
        } else if (response.status === 429) {
          toast.error(
            "Batas penggunaan AI tercapai. Mohon tunggu beberapa saat dan coba lagi.",
          );
        } else {
      ...
      if (error.name === "AbortError") {
        toast.error(
          "Permintaan dibatalkan karena terlalu lama. Coba file yang lebih kecil atau kategori.",
        );
      } else {
        toast.error(
  ...
  const handleSaveQuestions = async () => {
    try {
      ...
      const questionsToSave = questions.map((q) => ({
        ...
        category:
          category ||
          (selectedFile ? selectedFile.name.split(".")[0] : "Uncategorized"),
        status: "DRAFT",
      }));

      console.log(
        `[INFO] Menyiapkan ${questionsToSave.length} soal untuk disimpan`,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to save questions: ${response.status}`,
        );
      }
  ...
  return (
    <div className="flex h-screen bg-zinc-100 dark:bg-zinc-900 flex-col lg:flex-row">
      <Sidebar />
      <div className="flex-1 p-4 lg:p-6 xl:p-8 lg:ml-0 w-full transition-all duration-300">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="mt-4 text-cyan-500 dark:text-cyan-400 font-medium">
              Memuat...
            </p>
          </div>
        ) : (
          <div className="max-w-4xl mx-auto relative lg:pl-16 xl:pl-24 lg:pr-4 xl:pr-12">
            ...
            <div className="mb-8 mt-10 lg:mt-0 relative">
              ...
              <p className="text-slate-400 dark:text-zinc-400">
                Buat soal secara otomatis dengan bantuan AI
              </p>
            </div>
            ...
            {selectedFile && (
              <div className="mb-6 bg-blue-900/30 dark:bg-blue-900/50 border border-blue-700/50 dark:border-blue-800/50 rounded-xl p-4 text-sm text-blue-300 dark:text-blue-400 shadow-lg card-glass">
                <p className="flex items-start">
                  ...
                  <span>
                    <strong>Tip:</strong> Jika upload file gagal, pastikan file
                    Anda dalam format TXT, PDF, DOC, atau DOCX dan berukuran
                    kurang dari 10MB. Coba gunakan file dengan konten teks yang
                    dapat diakses.
                  </span>
                </p>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  {selectedFile && (
                    <p className="text-sm text-slate-400 dark:text-zinc-500">
                      Soal akan digenerate berdasarkan konten dari file{" "}
                      {selectedFile.name}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-center text-sm text-slate-400 dark:text-zinc-500">
                    {questionCount === 1 ? (
                      ...
                      <>
                        <span className="font-medium text-cyan-400 dark:text-cyan-500">
                          {questionCount}
                        </span>{" "}
                        soal akan dibuat
                  ...
                  {questionCount > 20 && (
                    <div className="text-center text-sm text-yellow-500 dark:text-yellow-400 bg-yellow-900/20 dark:bg-yellow-900/50 border border-yellow-800/50 dark:border-yellow-900/50 rounded-lg p-2">
                      ...
                      Generating banyak soal mungkin membutuhkan waktu lebih
                      lama
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col items-center justify-center text-center py-8">
                  ...
                  {selectedFile && selectedFile.size > 300000 && (
                    <div className="mt-4 p-3 bg-blue-900/30 dark:bg-blue-900/50 text-blue-300 dark:text-blue-400 text-sm rounded-lg border border-blue-800 dark:border-blue-900">
                      <p>
                        File Anda berukuran besar (
                        {Math.round(selectedFile.size / 1024)} KB). Proses
              ...
              <div className="mt-10 bg-slate-50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/60 rounded-xl p-5 lg:p-8 shadow-xl">
                ...
                <div className="space-y-6">
                  {questions.map((q, index) => (
                    <div
                      key={index}
                      className="p-5 bg-slate-100 dark:bg-zinc-800/60 rounded-xl border border-slate-200 dark:border-zinc-700/20 shadow-md hover-lift"
                    >
                      ...
                      <p className="text-slate-800 dark:text-zinc-100 mb-4 leading-relaxed">
                        {q.question}
                      </p>
                        ...
                        <div className="mb-5 pl-4 border-l-2 border-cyan-300 dark:border-cyan-800/50 rounded-sm py-1">
                          ...
                          <ul className="space-y-1">
                            {Object.entries(q.options).map(([key, value]) => (
                              <li
                                key={key}
                                className={`text-slate-700 dark:text-zinc-300 pl-2 py-1 rounded ${key === q.answer ? "bg-cyan-100 dark:bg-cyan-900/20 border-l-2 border-cyan-500" : ""}`}
                              >
                                <strong
                                  className={`${key === q.answer ? "text-cyan-600 dark:text-cyan-400" : "text-slate-500 dark:text-zinc-500"} mr-1.5`}
                                >
                                  {key}.
                                </strong>{" "}
                                {value}
                      ...
                      <div className="bg-slate-100 dark:bg-zinc-900/80 p-4 rounded-lg border border-slate-200 dark:border-zinc-800/80">
                        ...
                        <p className="text-sm text-slate-500 dark:text-zinc-400 mt-3 leading-relaxed">
                          <strong className="text-slate-700 dark:text-zinc-300 block mb-1">
                            Penjelasan:
                          </strong>{" "}
                          {q.explanation}
                        </p>
